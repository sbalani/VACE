...
from vace.configs import (VACE_IMAGE_PREPROCCESS_CONFIGS, VACE_IMAGE_MASK_PREPROCCESS_CONFIGS, 
                         VACE_IMAGE_MASKAUG_PREPROCCESS_CONFIGS, VACE_VIDEO_PREPROCCESS_CONFIGS, 
                         VACE_VIDEO_MASK_PREPROCCESS_CONFIGS, VACE_VIDEO_MASKAUG_PREPROCCESS_CONFIGS, 
...
class VACEComplete:
    ...
    def create_ui(self, *args, **kwargs):
        gr.Markdown("""
                    <div style="text-align: center; font-size: 24px; font-weight: bold; margin-bottom: 15px;">
                        <a href="https://ali-vilab.github.io/VACE-Page/" style="text-decoration: none; color: inherit;">VACE Complete</a>
                    </div>
                    """)
        
        with gr.Tabs(elem_id='VACE Complete') as vace_tabs:
            # Preprocessing Tab
            with gr.TabItem('Preprocessing', id=1, elem_id='preprocess_tab'):
                with gr.Tabs(elem_id='Preprocess Types') as preprocess_tabs:
                    with gr.TabItem('Video Tag', id=1, elem_id='video_tab'):
                        self.vace_video_tag.create_ui_video(*args, **kwargs)
                    with gr.TabItem('Image Tag', id=2, elem_id='image_tab'):
                        self.vace_image_tag.create_ui_image(*args, **kwargs)
                    with gr.TabItem('Composition Tag', id=3, elem_id='composition_tab'):
                        self.vace_tag_composition.create_ui_composition(*args, **kwargs)
                    with gr.TabItem('Video Tool', id=4, elem_id='video_tool_tab'):
                        self.vace_video_tool.create_ui_video_tool(*args, **kwargs)
            
            # Generation Tab
            with gr.TabItem('VACE Generation', id=2, elem_id='generation_tab'):
    ...
    def generate(self, output_gallery, src_video, src_mask, src_ref_image_1, src_ref_image_2, src_ref_image_3, prompt, negative_prompt, shift_scale, sample_steps, context_scale, guide_scale, infer_seed, output_height, output_width, frame_rate, num_frames):
        ...
        src_ref_images = [x for x in [src_ref_image_1, src_ref_image_2, src_ref_image_3] if x is not None]
    ...
    def set_callbacks(self, **kwargs):
        # Set callbacks for preprocessing tabs
        ...
        # Set callbacks for generation tab
        self.gen_inputs = [self.output_gallery, self.src_video, self.src_mask, self.src_ref_image_1, 
                          self.src_ref_image_2, self.src_ref_image_3, self.prompt, self.negative_prompt, 
                          self.shift_scale, self.sample_steps, self.context_scale, self.guide_scale, 
                          self.infer_seed, self.output_height, self.output_width, self.frame_rate, 
                          self.num_frames]
        ...
        self.generate_button.click(self.generate,
                                 ...
                                 queue=True)
        self.refresh_button.click(lambda x: self.gallery_share_data.get() if self.gallery_share else x, 
                                inputs=[self.output_gallery], 
                                outputs=[self.output_gallery])


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Argparser for VACE Complete:\n')
    ...
    with gr.Blocks() as demo:
        vace_complete = VACEComplete(args, skip_load=False, gallery_share=True, gallery_share_limit=5)
        vace_complete.create_ui()
        vace_complete.set_callbacks()
